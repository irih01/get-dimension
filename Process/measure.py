...
class MeasureMath:
    ...
    def area(self, contour):
        ...
        # OPTIMIZARE: Calculăm direct pe baza pătratului raportului mm_per_px salvat
        mm2 = (self.mm_per_px ** 2) * px2 if self.mm_per_px else None
...
class MeasureView:
    ...
    def draw_dim_line(frame, p1, p2, val, color=(0, 255, 0), thickness=2, font_size=0.7, unit="mm", offset=20):
        ...
        # FIX TEXT: Verificăm tipul valorii pentru a asigura formatarea corectă
        if isinstance(val, (int, float, np.floating)):
            text = f'{val:.1f} {unit}'
        else:
            text = f'{val}'

        # Sageti (FIX: Ajustat tipLength dinamic pe baza lungimii liniei pentru vizibilitate crescută)
        tip_len = max(0.05, min(0.3, 15.0 / length))
        cv.arrowedLine(frame, p1_off, p2_off, color, thickness, tipLength=tip_len)
        cv.arrowedLine(frame, p2_off, p1_off, color, thickness, tipLength=tip_len)

        # Text
        mid = ((p1_off[0] + p2_off[0]) // 2, (p1_off[1] + p2_off[1]) // 2)
        
        (tw, th), _ = cv.getTextSize(text, cv.FONT_HERSHEY_SIMPLEX, font_size, 1)
        ...
        # Text propriu-zis
        cv.putText(frame, text, txt_pos, cv.FONT_HERSHEY_SIMPLEX, font_size, color, 1, cv.LINE_AA)
